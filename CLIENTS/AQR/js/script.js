...
// Hero Slider with Ken Burns Effect - Optimized
let currentSlide = 0;
...
function nextSlide() {
    ...
    // Lazy load next image if not loaded
...
// Wait for first image to load before starting slider
if (slides.length > 0) {
    const firstImg = slides[0].querySelector('img');
    if (firstImg) {
        if (firstImg.complete) {
            // Image already loaded, start slider
            setTimeout(() => setInterval(nextSlide, 6000), 2000);
        } else {
