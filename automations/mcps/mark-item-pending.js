 ...
 * Marks a decision, task, or note as pending (shows in dashboard again)
 */
...
module.exports = {
  description: 'Marks a decision, task, or note as pending (shows in dashboard again)',
  ...
  params: {
    ...
    item_text: {
      type: 'string',
      required: true,
      description: 'The exact text of the item to mark as pending'
    }
  },
  ...
  async execute(params, { PROJECT_ROOT }) {
    ...
    status[item_type][key] = {
      ...
      item: item_text,
      marked_pending_at: new Date().toISOString()
    };
    ...
    return {
      success: true,
      message: `Marked ${item_type.slice(0, -1)} as pending`,
      item_type,
      item_text,
      status: 'pending',
      note: 'This item will appear in dashboard again on next Daily Ops run.'
    };
  }
};
