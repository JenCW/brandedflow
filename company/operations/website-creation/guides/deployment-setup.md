...
## Automatic Deployment (Git → Netlify)
...
**If you see a GitHub URL** (like `https://github.com/yourusername/repo.git`), you're good! Skip to Step 2.
...
5. **Configure Build Settings**
   ...
   - **Base directory:** Leave blank (or type the subdirectory if your website is in a subfolder)
   - **Build command:** Leave blank (no build needed for static HTML)
   - **Publish directory:** Type the folder containing your website files (e.g., `clients/client-name/03_website` or just `.` if deploying from the website folder)
   
   **Important:** If your whole repo contains multiple projects but the website is in a subfolder:
   - **Base directory:** `clients/client-name/03_website`
...
1. **Authenticate with Netlify CLI**
   ```bash
   cd /path/to/website
   netlify login
   ```
...
2. **Initialize & Deploy**
   ...
   **You'll be asked:**
   ...
   - "Site name" → Use a descriptive name
